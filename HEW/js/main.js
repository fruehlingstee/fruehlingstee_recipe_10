...
const recipes = [
 ...
 {id:48,name:"やみつき明太ひじきごはん",ingredients:["白米","にんじん","明太子","乾燥芽ひじき","めんつゆ"],img:"images/やみつき明太ひじきごはん.jpg",steps:["乾燥芽ひじきはたっぷりの水で戻す。","戻したひじきの水気を絞る。にんじんは細切り、明太子はほぐす。大きめのボウルに加え、めんつゆ2倍濃縮大さじ２、水大さじ１を加えて混ぜ、ふんわりラップをして600Wで3分加熱する。","取り出してよく混ぜたら、混ぜ込みごはんの素完成。","ごはんを加えて混ぜ込む。"]},
...
document.addEventListener("DOMContentLoaded", function(){
  ...
  if(savedIng){
    ...
    if(input){
      ...
      addIng();
      localStorage.removeItem("rouletteIngredient");
    }
  }
});
...
function initIndex(){
  ...
  tags.innerHTML = sel.map((s,i) =>
    `<span class="tag">${s}<span onclick="removeIng(${i})">×</span></span>`
  ).join("");
  ...
  grid.innerHTML = recipes
    .filter(r => sel.some(s => r.ingredients.includes(s)))
    .map(r => `
...
/* =====================
   detail
   ※ 閲覧数は Firestore へ（detail.html の module script で処理）
   ※ initDetail() はレシピ表示のみ担当
===================== */
...
function initDetail(){
  ...
  document.getElementById("rImg").src = r.img;

  document.getElementById("rIngs").innerHTML = r.ingredients
    .map(i => `<li>${i} <button class="shop-btn" onclick="addShop('${i}')">＋</button></li>`)
    .join("");

  document.getElementById("rSteps").innerHTML = r.steps
    .map(s => `<li>${s}</li>`)
    .join("");
}

/* addShop / toggleFav はdetail.html の module script で Firestore 版に上書きされる */
function addShop(t){
  // フォールバック（未ログイン時）
  let s = Storage.getShop();
  if(!s.find(x => x.text === t)) s.push({text:t, checked:false});
  Storage.saveShop(s);
  alert("追加しました（未ログイン：ローカル保存）");
}
...
function initMyPage(){
  ...
  if(sh){
    ...
    sh.innerHTML = s.map((x,i) =>
      `<div class="list-item ${x.checked?"checked":""}" onclick="toggleShop(${i})">${x.text}</div>`
    ).join("") || "空";
  }
...
/* =====================
   ranking
   ※ ranking.html の module script で Firestore 版が上書きするため
     この関数は ranking.html では実質呼ばれない
===================== */

function initRanking(){
  // ranking.html では module script が代わりに描画する
}
...
window.onload = () => {
  initIndex();
  initDetail();
  initRanking(); // ranking.html では module script が上書き
};
